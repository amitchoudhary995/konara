        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl relative z-10">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-6 mb-6">
                ...
                <div className="lg:col-span-5 pr-0 lg:pr-12">
                    ...
                    <form className="relative max-w-sm group" onSubmit={(e) => e.preventDefault()}>
                        ...
                        <input 
                            ...
                            className="w-full bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-full pl-5 pr-28 py-3 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-white/35 focus:outline-none focus:border-secondary/60 focus:bg-white dark:focus:bg-white/10 transition-all duration-500 shadow-inner group-focus-within:ring-2 group-focus-within:ring-secondary/20"
                        />
                        <button className="absolute right-1 top-1 bottom-1 bg-gradient-to-r from-secondary to-orange-500 hover:from-orange-500 hover:to-secondary text-white font-bold rounded-full px-4 sm:px-5 transition-all duration-300 shadow-[0_0_10px_rgba(227,77,42,0.2)] hover:shadow-[0_0_15px_rgba(227,77,42,0.5)] flex items-center gap-1.5 uppercase tracking-wider text-[10px] group/btn">
                            <span className="hidden sm:inline">Subscribe</span>
                            <Mail size={12} className="sm:hidden group-hover/btn:scale-110 transition-transform" />
                        </button>
                    </form>
                </div>
                ...
                <div className="lg:col-span-7 grid grid-cols-2 sm:grid-cols-3 gap-6 sm:gap-4 pt-1">
                    ...
                    <div>
                        ...
                        <ul className="space-y-2">
                            {[
                                ...
                                <li key={link.name}>
                                    <Link to={link.path} className="group flex items-center text-slate-500 dark:text-white/50 hover:text-secondary dark:hover:text-white transition-colors duration-300 font-medium text-xs">
                                        <ArrowRight size={12} className="text-secondary opacity-0 -ml-4 mr-0 group-hover:opacity-100 group-hover:mr-2 group-hover:ml-0 transition-all duration-300" />
                    ...
                    {/* Column 2 */}
                    <div>
                        <h4 className="text-slate-800 dark:text-white font-bold mb-2 uppercase tracking-widest text-xs dark:drop-shadow-md">Explore</h4>
                        <ul className="space-y-2">
                            {[
                    ...
                    {/* Column 3 - Socials */}
                    <div>
                        <h4 className="text-slate-800 dark:text-white font-bold mb-3 uppercase tracking-widest text-xs dark:drop-shadow-md">Connect With Us</h4>
